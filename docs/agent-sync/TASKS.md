...
如果你開工時字型檔還不在,**先做 A 和 D,B/C 等檔案到齊再做**,
...
`lib/screens/review_screen.dart`,`_buildDoneMode`(約第 380–430 行)。

目前是靠上對齊、下面一大片空白。改成整頁垂直置中:
外層的 `Column` 加 `mainAxisAlignment: MainAxisAlignment.center`,
並確保它被撐滿整個可用高度。

**內容一個都不增減**,維持現有的:完成文案 → 累計字數 → 回首頁按鈕
(里程碑時多一個 🎉 動畫,那個維持原樣)。

**不要加任何統計數字**(答對幾題、花了多久、正確率一律禁止)。
