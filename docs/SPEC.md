...
## 14. 視覺改版:首頁背景金屬珍珠(v10 新增)

> 本節是視覺改版的第一批規格。**只涵蓋首頁背景裝飾層,不動任何既有版面、
> 字級、顏色或元件。** 其餘畫面的視覺規格待 Stitch 設計稿定案後另行補上。

### 14.1 這是什麼

首頁背景加入 4 顆**銀色金屬珍珠**作為裝飾。它們是靜態圖片素材(非程式繪製),
以極慢速度上下飄浮,用來讓背景不再是一片空白,並建立整個 App 的「流動感」
設計語言。
...
素材已經放好並在 `pubspec.yaml` 註冊完成,實作者不需要再處理素材:

```
assets/images/pearl.png          200×200  (1x)
assets/images/2.0x/pearl.png     400×400  (2x)
assets/images/3.0x/pearl.png     600×600  (3x)
```

透明背景 PNG,正圓形,已去背無灰邊。

用 `Image.asset('assets/images/pearl.png', width: n, height: n)` 載入即可,
Flutter 會依螢幕密度自動挑 2x / 3x,**不要手動指定 2.0x 或 3.0x 的路徑**。

### 14.3 位置與大小

座標以**畫面可用區域**(扣掉 status bar 後)左上角為原點。
水平用畫面寬度百分比,垂直用畫面高度百分比,確保各種螢幕比例都合理。

| # | 直徑 | 水平位置 | 垂直位置 | 說明 |
|---|------|---------|---------|------|
| 1 | 200 | left: -70px | top: 12% | 被左邊緣裁切,約露出 2/3 |
| 2 | 140 | right: -70px | top: 55% | 被右邊緣裁切,約露出 1/2 |
| 3 | 90  | left: 8%    | top: 74% | 完整可見 |
| 4 | 56  | right: 14%  | top: 20% | 完整可見 |

刻意不對稱、大小差距明顯、兩顆被邊緣裁切。**不要改成平均分布或左右對稱。**
...
珍珠**永遠在最底層之上、內容之下**,而且必須設 `IgnorePointer`,
確保不會攔截到中央氣泡或 START 按鈕的點擊。

珍珠 #3 的位置刻意避開 START 按鈕與「You know N words」文字。
若在小螢幕(高度 < 600dp)上實測發現壓到文字,把 #3 的 `top` 調到 78%,
不要改文字的位置。
...
- 曲線用 `Curves.easeInOut`,**不要用 linear**,linear 會看起來像機械式往返
- 用 `AnimationController(...)..repeat(reverse: true)`
- 四顆的週期刻意互質,避免同步成呼吸般的整齊起伏
- 起始相位錯開(可用不同的 `controller.value` 初始值,或延遲不同秒數啟動)
- **只做垂直位移。** 不旋轉、不縮放、不改透明度

### 14.6 效能與無障礙

- 四個 controller 在 `dispose()` 時必須全部釋放
- 只讓珍珠自己重繪,不要讓整個首頁跟著 rebuild
  (用 `AnimatedBuilder` 的 `child` 參數把 `Image.asset` 傳進去快取)
- 若 `MediaQuery.of(context).disableAnimations` 為 true,直接顯示靜止的珍珠
  不啟動 controller

### 14.7 驗收

1. 首頁看得到 4 顆銀色金屬珍珠,大小明顯不同、分布不對稱
2. 其中 2 顆被畫面左右邊緣裁切
3. 珍珠邊緣乾淨,沒有灰色外圈或方形邊界
4. 珍珠緩慢上下飄浮,四顆節奏不一致,看不出同步
5. 點擊珍珠所在位置,若下方有按鈕仍能正常觸發
6. 珍珠沒有蓋住任何文字或按鈕
7. 中央主氣泡、字級、顏色、間距與改版前完全相同
8. `flutter test` 全綠

## 15. 視覺設計系統(v10 定案)

> 本節是整個 App 視覺的唯一來源。**所有顏色、字型、圓角一律引用本節的 token,
> 不准在 widget 裡寫死色碼。** 設計稿由 Stitch 產出、Shawn 確認。

### 15.1 色彩 token

| Token | 色碼 | 用途 |
|---|---|---|
| `primary`   | `#22243A` | 深墨色。主要文字、主要按鈕底色 |
| `secondary` | `#E3E1EC` | 淡紫霧。次要面板、分隔、非活躍狀態 |
| `tertiary`  | `#BCC0D2` | 鉻灰。輔助文字、圖示、進度條底槽 |
| `neutral`   | `#F0EEF5` | 銀白。頁面背景 |
| `surface`   | `#FFFFFF` | 卡片、選項按鈕底色 |

狀態色(僅用於答題回饋):

| Token | 色碼 | 用途 |
|---|---|---|
| `correct`   | `#A9DED2` 底 / `#1F5C4D` 字 | 答對的選項 |
| `incorrect` | `#F0C2DE` 底 / `#7A2049` 字 | 答錯時使用者選的那個選項 |

**除上表外不得引入任何新顏色。**

### 15.2 字型

兩套 Google Fonts,**以 TTF 檔打包進 `assets/fonts/`,不使用 `google_fonts` 套件**。
理由:不引入新套件,且離線一定顯示得出來(郵輪無網路)。

| 角色 | 字型 | 用在哪 |
|---|---|---|
| Headline | **Syne** | 單字本身、大數字、畫面大標題 |
| Body     | **Hanken Grotesk** | 定義、例句、按鈕文字、一般內文 |
| Label    | **Hanken Grotesk** | 小標籤(全大寫 + 寬字距) |

字級與字重:

| 名稱 | 字型 | 大小 | 字重 | 備註 |
|---|---|---|---|---|
| `displayLarge` | Syne | 64 | 800 | 首頁轉盤數字 |
| `displayMedium`| Syne | 34 | 800 | 「You know 87 words」、Library 標題 |
| `headline`     | Syne | 28 | 700 | 單字本身 |
| `body`         | Hanken Grotesk | 15 | 400 | 定義、例句、選項 |
| `bodyStrong`   | Hanken Grotesk | 15 | 600 | 按鈕文字 |
| `label`        | Hanken Grotesk | 11 | 600 | 全大寫,字距 `letterSpacing: 1.6` |
| `caption`      | Hanken Grotesk | 13 | 400 | 音標、輔助說明 |

### 15.3 形狀與間距

- 卡片圓角 `20`,選項按鈕圓角 `14`,主按鈕為膠囊狀(圓角 = 高度的一半)
- 主按鈕高度 `52`,選項按鈕高度 `56`
- 畫面左右安全邊距 `20`
- 卡片內距 `20`,元素垂直間距基準 `12 / 20 / 32` 三級
- 陰影只用一種:`BoxShadow(color: #22243A 6%, blurRadius: 24, offset: (0, 8))`

### 15.4 逐畫面調整(相對於 Stitch 設計稿)

Stitch 的稿有幾處與產品規則衝突或內容是假的,**以下以本節為準**:

**首頁(兩個狀態)**
- 背景加 4 顆金屬珍珠,規格見第 14 節。Stitch 稿上沒有,由 Flutter 疊加
- 其餘照稿

**Library**
- 牌組進度條與百分比 **保留**(CLAUDE.md 例外二)
- 設計稿上的「MODULE 01 / 02 / 03」標籤是 Stitch 自己編的,不要照抄
- 牌組清單本來就是從 DB 動態讀的(`repo.allDecks()`),維持現狀即可,
  **牌組名稱不得寫死**
- **拿掉底部的「GENERATE WITH AI」按鈕。** 金鑰目前在前端會暴露,
  上線前必須改成後端代理(見第 11 節與 CLAUDE.md 末段)。
  `lib/services/ai_service.dart`、`lib/screens/generate_screen.dart`
  與其中的警告註解**全部保留不動**,只是 UI 不露出入口

**學習完成畫面(Study Complete)**
- Stitch 稿是靠上對齊、下半部大片空白。**改成整頁垂直置中。**
- 內容只有三樣:「Done for today」(caption 灰)、「You know N words」
  (displayMedium)、HOME 按鈕。不得增加任何統計數字

**學習畫面**
- 題目卡的例句要把目標單字挖空成底線,如 Stitch 稿所示
- 「I FORGOT」維持虛線外框的獨立樣式,和四個選項在視覺上明確區隔
- 介面文字(按鈕、標題、提示)全英文,不出現「review」字樣
- **但四選一的選項與卡片釋義維持中文。** Shawn v10 裁定:
  英→中的方向難度剛好,改成英英釋義會拉高錯誤率,違反產品原則第 4 條

### 15.5 實作方式

- 全部集中在 `lib/theme/app_theme.dart`(新檔),匯出 `ThemeData` 與
  一個 `AppColors` / `AppTextStyles` 常數類
- 各畫面改用 `Theme.of(context)` 或上述常數,**移除所有寫死的色碼與 TextStyle**
- 字型檔放 `assets/fonts/`,在 `pubspec.yaml` 的 `fonts:` 區塊註冊

### 15.6 驗收

1. 全專案 grep 不到寫死的十六進位色碼(除了 `app_theme.dart` 本身)
2. Library 顯示 7 個真實牌組,沒有「MODULE」字樣,沒有 GENERATE WITH AI 按鈕
3. 學習完成畫面內容垂直置中
4. 斷網狀態下字型仍正確顯示(代表沒有走網路載字型)
5. 介面字串搜尋不到「複習」與「review」(`review_screen.dart` 這個檔名可以留)
6. `flutter test` 全綠

## 16. 虹彩流動背景與金屬圖示(v11 新增)

> 本節取代原本「頁面背景是單色 `neutral`」的設定。
> 第 15.1 節的 `neutral` 仍是所有色彩運算的基準色,只是不再直接當背景填色。
>
> **本節每一個數字都是實測調出來的,不是估的。** 不要自行微調,
> 覺得哪裡不對就寫進 `QUESTIONS.md`。

### 16.0 為什麼不用 shader

即時算虹彩最正統的做法是 GLSL fragment shader,但
**Flutter Web 的 CanvasKit 至今仍未支援 `FragmentProgram`**
(flutter/flutter#114121 仍開著)。本專案目標平台就是 Web PWA,
shader 這條路直接排除。

改用「預先算好的材質 + 兩層相對運動」達到等效效果。
**不要為了這個效果引入任何第三方套件**,下面的做法只用 Flutter 內建能力。

### 16.1 材質資產

已產生好並註冊完成,**實作者不需要處理素材,也不要重新產生或修改**:
...
兩張都是薄膜干涉演算法產生的虹彩材質。三件事已經預先烘進素材裡:

1. **對比**已經拉好,執行時**不需要**再套 `ColorFiltered` 調對比
2. **細顆粒抖動**(σ≈0.0016)已經加好,用來消除 8-bit 漸層的色階斷帶
3. 色域已壓在 `neutral` 附近,確保深墨色文字疊上去仍有足夠對比

**不要對這兩張圖套 `ColorFiltered`、`Opacity` 以外的任何濾鏡,
也不要調整顏色、亮度、飽和度。**

### 16.2 流動的原理

單獨一張靜態圖平移只會像「滑動的壁紙」。真正的流動感來自
**兩層材質以不同方向、不同速度移動,再用 soft light 混合**——
兩層的干涉會讓花紋本身持續變形,而不只是位移。

我用 5 個時間點驗算過:t=0 與 t=0.4 的畫面平均差異 5.76/255,
畫面標準差 7.31(跟 Shawn 確認過的靜態版本 7.68 幾乎一致),
四邊 3px 的最低亮度 206(沒有黑邊)。
...
**只用一個** `AnimationController`:

```dart
AnimationController(vsync: this, duration: const Duration(seconds: 60))..repeat();
```

- 是 `repeat()`,**不是** `repeat(reverse: true)`。整個運動用三角函數
  構成,本身就是週期封閉的,反向播放會出現折返感
- 四顆珍珠與中央氣泡各自有自己的 controller,**不要共用**

#### 兩層的變換

設 `t = controller.value`(0→1),`a = 2 * pi * t`:

| | 過掃描 | 額外縮放 | 水平位移 | 垂直位移 | 旋轉 |
|---|---|---|---|---|---|
| A 層 | ×1.6 | `1.0 + 0.05 * sin(a)` | `70 * sin(a)` | `110 * cos(0.7a)` | 無 |
| B 層 | ×1.6 | 無 | `-70 * cos(0.8a)` | `-110 * sin(0.6a)` | `4° * sin(0.5a)` |

**過掃描 ×1.6 是必要的,不是保險。** 我第一版沒有加,結果 B 層旋轉時
四個角會轉出畫面外,邊緣出現硬邊和空白——那正是「粗糙」的來源。
1.6 倍是實測後能同時容納 ±70px 位移與 4° 旋轉的最小值,再小會露邊。

實作方式:先 `BoxFit.cover` 鋪滿,再乘上 1.6 的 `Transform.scale`,
所有位移與旋轉都發生在這個放大後的畫布內,最後由外層 `ClipRect` 裁掉。

位移單位是**邏輯像素**(dp),不是物理像素,也不是百分比。
...
- `BlendMode.softLight`
- B 層不透明度 **0.5**
- 最外層墊一個 `Container(color: AppColors.neutral)`,避免圖片載入前閃白

**執行時不要再做對比調整。** 我實測過:對比預先烘進素材、執行時不套
`ColorFiltered`,畫面標準差 6.90,跟套了後製對比的 7.68 幾乎一樣,
但少一道每幀的全畫面運算,而且**不會把 8-bit 的色階斷帶一起放大**。
這是避免顆粒感的關鍵之一。

### 16.4 避免顆粒感與卡頓(重要)

這一節每一條都是為了「動起來要滑順、不要粗糙」而存在,不要省略。

1. **`filterQuality: FilterQuality.medium`**
   兩層的 `Image.asset` 都要明確指定。預設的 `low` 是最近鄰等級,
   縮放與旋轉時會產生閃爍的鋸齒(視覺上就是「顆粒感」)。
   不要用 `high`,那在 web 上成本明顯偏高而這裡看不出差別。

2. **位移量一律用 `double`,絕對不要 `round()` / `toInt()`**
   60 秒一圈、位移只有 ±70px,換算下來每幀位移不到 0.02px。
   一旦取整,畫面會變成「每隔幾秒跳一格」的階梯狀移動,
   那是最典型的卡頓感來源。子像素位移由 `Transform` 自己插值處理。

3. **整個背景包一層 `RepaintBoundary`**
   否則每幀都會把首頁其他內容一起標記重繪,在低階手機上會掉幀。

4. **`AnimatedBuilder` 的 `child` 參數一定要用**
   把兩個 `Image.asset` 當 `child` 傳進去快取,`builder` 每幀只重建
   `Transform`。不要在 `builder` 裡面呼叫 `Image.asset`。

5. **不要在執行時套 `ImageFilter.blur` 或 `BackdropFilter`**
   模糊已經烘進素材了。執行時做全畫面模糊在 web 上非常貴,是掉幀主因。

6. **不要自己加雜訊或 dither**
   素材裡已經有 σ≈0.0016 的細顆粒。再加一層只會變成看得見的雜點。

### 16.5 圖層順序(更新 14.4 節)

由後到前:

1. `AppColors.neutral` 底色
2. **虹彩流動背景**(本節)
3. 金屬珍珠(第 14 節)
4. 中央主氣泡、文字、按鈕、底部導覽列

第 2、3 層都必須包 `IgnorePointer`。

### 16.6 中央主氣泡也要飄浮

Stitch 設計稿裡中央氣泡是會小幅上下浮動的,之前漏掉,補上:

- 位移 **±6px**,單趟週期 **19 秒**,`Curves.easeInOut`,`repeat(reverse: true)`
- 19 秒刻意跟四顆珍珠的 22/18/25/15 都不同,避免視覺上對齊成同一個節拍
- **只做垂直位移。** 不旋轉、不縮放、不改透明度
- **拉霸轉動中(`_rolling == true`)時暫停飄浮**,不然兩個動畫會打架

### 16.7 拉霸圓圈:拿掉「SPIN」文字

**圓圈裡不要出現任何文字。** 目前「SPIN」那行字拿掉,只留一個金屬圖示。

| 狀態 | 圓圈內容 |
|---|---|
| 還沒轉 | 金屬骰子圖示,尺寸 **56**,置中,**沒有任何文字** |
| 轉動中 | 大數字(維持現狀) |
| 轉完 | `NEW TODAY` 小標 + 大數字(維持現狀) |

圖示用 `Icons.casino_outlined`。

**可觸碰性的補償:** 拿掉文字後少了「這裡可以點」的提示,
所以未轉狀態的圖示要加一個極輕微的呼吸動畫——
縮放 `1.0 ↔ 1.04`,單趟 **2.4 秒**,`Curves.easeInOut`,`repeat(reverse: true)`。
**只有未轉狀態有這個動畫**,轉完之後不要。

### 16.8 金屬圖示

彩色 emoji(🎰 拉霸、🎉 慶祝)跟整體調性衝突,全部換掉。

**做法:Material 內建圖示 + `ShaderMask` 疊銀色漸層。**
不新增圖片素材、不引入套件、任何尺寸都不會糊。

| 位置 | 圖示 | 尺寸 |
|---|---|---|
| 首頁拉霸(未轉) | `Icons.casino_outlined` | 56 |
| 完成畫面里程碑 | `Icons.auto_awesome` | 40 |

漸層:`LinearGradient`,`begin: Alignment.topLeft`,`end: Alignment.bottomRight`
...
我把三組色階實際畫在虹彩背景上比對過:直接沿用珍珠那組
(最深只到 `#6E7484`)在淺色背景上幾乎看不見,對比嚴重不足。
小尺寸圖示需要比大面積物件更強的明暗差才讀得出形狀,
所以圖示這組刻意把暗部壓到 `#22243A`。

**這兩組色階不要互換,也不要統一。** 珍珠用第 14 節的素材原圖,
圖示用上面這組。

#### 實作注意

- **`ShaderMask` 的 `blendMode` 要用 `BlendMode.srcATop`**,
  用 `srcIn` 會把圖示的透明區域一起填掉
- 抽成一個小 widget(例如 `ChromeIcon(icon, size)`)重複使用,
  不要在兩個地方各寫一次漸層

### 16.9 網頁版寬度上限

目前 web 版沒有寬度限制,按鈕會整條拉滿貼到瀏覽器邊緣。

在 `lib/main.dart` 的 `MaterialApp.builder` 統一包一層:
...
430 是設計稿的基準寬度。超出的區域填 `AppColors.neutral`。
**這是全域設定,不要在每個畫面各包一次。**

### 16.10 無障礙

`MediaQuery.disableAnimations` 為 true 時:

- 背景停在 `t = 0` 的靜止畫面(controller 不啟動)
- 中央氣泡不飄浮
- 拉霸圖示不呼吸
- 四顆珍珠不飄浮(第 14.6 節已有)
...
60 秒一圈是刻意的慢速。**不要因為「看不太出來在動」就加快。**

產品原則第 1 條是零壓力、不搶注意力。背景一旦快到會被注意到,
就變成干擾。正確的體感是:盯著看 30 秒會發現它變了,
但正常使用時不會察覺。

### 16.12 驗收

1. 首頁背景不是單色,看得到虹彩流動的材質
2. 盯著看 30 秒,花紋會**變形**,不是整片平移
3. **畫面四邊沒有硬邊、黑邊或空白**(過掃描沒做足會出現)
4. 動起來滑順,沒有一格一格跳、沒有閃爍的雜點、沒有色階斷帶
5. 金屬珍珠疊在背景上,材質看起來屬於同一個家族
6. 所有文字與按鈕仍然清楚可讀
7. 拉霸圓圈裡**沒有任何文字**,只有一個銀色金屬骰子圖示,且會極輕微呼吸
8. 完成畫面的慶祝圖示是銀色金屬,畫面上沒有任何彩色 emoji
9. 瀏覽器視窗拉到很寬時,內容維持在畫面中央 430px 寬
10. 中央氣泡會極慢上下飄浮,拉霸轉動時暫停
11. 開啟系統的「減少動態效果」後,以上動畫全部停止且畫面正常
12. `flutter test` 全綠
