...
### 6.1 首頁 `lib/screens/home_screen.dart`(重寫)

**如果今天還沒拉霸:**

畫面中央顯示一個大的拉霸按鈕。點下去 → 導向拉霸畫面。
...
「待複習:12 張」是中性的資訊陳述,可以顯示。但不要加上任何催促文案。

### 6.2 拉霸畫面 `lib/screens/daily_roll_screen.dart`(全新)

流程:

1. 進入畫面 → 顯示一個未轉動的轉盤/拉霸機圖示 + 「轉動」按鈕
2. 使用者點「轉動」→ 播放約 1.5 秒的滾動動畫(數字快速跳動)
3. 動畫結束 → 定格在結果數字
4. 依結果顯示不同文案:
...
|---|---|
| quota > 0 | 「今天的新單字:**4** 個」 |
| isJackpot == true | 「🎉 JACKPOT!今天放假,不用背新字!」+ 慶祝動畫 |
| quota == 0 且 wasCapped | 「今天先把之前的複習完就好」(不要有慶祝動畫) |

5. 底部按鈕「開始學習」→ 執行 5.5 的引入流程 → 導向複習畫面

**動畫實作提示:** 用 `AnimatedBuilder` + `Timer.periodic` 快速更換顯示數字即可,不需要引入動畫套件。

### 6.3 題目卡片元件 `lib/widgets/question_card.dart`

**【v3 修訂】** 原本規格是可翻面的 `flashcard.dart`。翻卡模式已被否決,
`lib/widgets/flashcard.dart` 應**刪除**(git 歷史保留即可,不要留死 code)。
改為這個不翻面的題目卡片元件。
...
注意:`meaning` 和 `exampleZh` **不由這個元件顯示**,由複習畫面在答題後另行呈現。

### 6.4 複習畫面 `lib/screens/review_screen.dart`

**【v3 定案】四選一測驗 + 獨立「忘了」按鈕 + 反應時間自動分級。**

所有卡片一律使用同一種模式,**不依 `repetitions` 分流**(v2 的分流設計已作廢)。
...
**反應時間分級的用意:** 使用者完全感覺不到這層機制,不需要額外操作,
但猶豫很久才答對的字會被判定為「還不熟」,排近一點再出現,
把 SM-2 的中間評分(3/4)救回來。
...
- 每題獨立計時,切換題目時重置
- 不需要處理「使用者切走再切回來」的情況(時間會很長 → 判 quality 3,這是合理的保守結果)

#### 答題後的回饋

1. 鎖定所有按鈕,不可再點
2. 正確答案標綠;若選錯,使用者選的那個標紅
3. **顯示 `exampleZh`(例句中譯)**,讓使用者確認語境理解
4. 停留 1.4 秒後自動跳下一題

按「忘了」時同樣走這個流程(標出正確答案 + 顯示中譯),不要跳過。
...
2. **排除語意過近的字**。近義詞會造成不公平的假陰性,污染排程訊號。
   實作方式:在 `starter_deck.json` 的卡片可加選填欄位 `avoidWith: ["postpone"]`,
   標記不可與哪些 word 同時出現在選項中。沒有這個欄位就不排除。
...
1. 進入時依 5.6 撈出佇列
2. 取得 quality 後:
   - 用該卡目前的 SM-2 狀態與 quality 呼叫 `reviewCard()`
   - 把回傳的新狀態寫回資料庫(easiness, interval, repetitions, dueDate),同時更新 `lastReviewed = now`
   - 前進到下一張
3. 佇列清空後 → 顯示完成畫面

**完成畫面文案:** 「今天的複習做完了」+ 返回首頁按鈕。不要顯示分數、正確率、或任何評比。

**中途離開:** 允許隨時返回首頁,已評分的卡片保留結果,未評分的維持原狀(明天會變積壓,由保護機制處理)。

### 6.5 牌組列表 `lib/screens/decks_screen.dart`(全新)
...
| `lib/screens/daily_roll_screen.dart` | 新增 | 見 6.2 |
...
5. 同一天重新整理頁面,不能再拉第二次
6. 引入的新卡數量與骰出的數字一致
7. 複習畫面:四選一 + 獨立「忘了」按鈕;例句中的目標單字有粗體;答題後標示正確/錯誤並顯示例句中譯
8. 評分後卡片的 dueDate 有按 SM-2 更新
9. 關掉瀏覽器重開,資料還在(驗證 Drift Web 持久化正常)
